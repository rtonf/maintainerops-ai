...
;// CONCATENATED MODULE: ./src/actionArgs.ts
function buildActionArgs(env) {
...
async function fetchPullRequestWorkItem(repo, pullNumber) {
    const pr = await githubJson(`/repos/${repo}/pulls/${pullNumber}`);
...
function analyzeOffline(item) {
    ...
    if (!hasTests && item.kind === "pull_request")
...
function buildSummary(item, hasSecuritySignal, hasTests, largeChange) {
    ...
    if (hasSecuritySignal)
        parts.push("It touches security-sensitive language or paths.");
    if (largeChange)
        parts.push("The change is large enough to deserve extra review time.");
    if (item.kind === "pull_request" && !hasTests)
        parts.push("No test file was detected in the changed file list.");
    return parts.join(" ");
}
function recommendedAction(item, hasSecuritySignal, hasTests, largeChange) {
    if (hasSecuritySignal)
        return "needs_security_review";
    if (item.kind === "issue")
        return "needs_more_info";
    if (!hasTests || largeChange)
...
function buildCommentDraft(item, hasSecuritySignal, hasTests, isFeedbackRequest) {
    ...
    const requests = [];
    if (!hasTests)
        requests.push("add tests or explain why this change is not testable");
    if (hasSecuritySignal)
        requests.push("call out the security boundary and any attacker-controlled inputs");
...
function buildEvidence(item, hasSecuritySignal, hasTests, largeChange) {
    const evidence = [
...
function assertAssessment(value) {
    ...
    const requiredStringFields = ["summary", "riskLevel", "recommendedAction", "commentDraft"];
...
function truncateForModel(input, maxChars = 120_000) {
...
function normalizeAssessmentLabels(assessment) {
...
async function assessWorkItem(item, options) {
...
function formatAssessment(item, assessment, format) {
    if (format === "json") {
