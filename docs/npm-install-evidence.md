...
The registry reports `0.1.8` as deprecated with instructions to use `0.1.9` or later. Version `0.1.13` was published through npm Trusted Publishing with provenance after the no-key demo release.
...
- npm resolved `maintainerops-ai@0.1.13` from the public registry.
...
Version `0.1.9` added a `prepack` script that rebuilds the CLI and Action runtime during both `npm pack` and `npm publish`. Version `0.1.13` preserves that guard and was published through the Trusted Publishing workflow.

## Trusted Publishing Evidence

- GitHub Release: `https://github.com/rtonf/maintainerops-ai/releases/tag/v0.1.13`
- Successful workflow run: `https://github.com/rtonf/maintainerops-ai/actions/runs/28727350922`
- npm latest: `0.1.13`
